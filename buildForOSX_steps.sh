...
# 13) This is the end of "regular" install (Carnets)

echo "Creating install_regular" >> $PREFIX/make_install_osx.log 2>&1
rm -rf install_regular  >> $PREFIX/make_install_osx.log 2>&1
mkdir -p install_regular  >> $PREFIX/make_install_osx.log 2>&1
cp -r Library install_regular  >> $PREFIX/make_install_osx.log 2>&1

# if you don't want scipy and anything that depends on scipy: 
# exit 0

# 14) Scipy (+ seaborn and gym)
sh ./scripts/scipyPackages.sh >> $PREFIX/make_install_osx.log 2>&1

# protobuf, coremltools and mlx (mlx says it requires M1 chips and OSX)
sh ./scripts/coreMLToolsPackages.sh >> $PREFIX/make_install_osx.log 2>&1

# scikit-learn
# Mystery: the script works if launched separately, doesn't if launched inside this script.
sh ./scripts/scikitLearnPackages.sh >> $PREFIX/make_install_osx.log 2>&1

# qutip 
sh ./scripts/qutipPackages.sh >> $PREFIX/make_install_osx.log 2>&1

# cartopy
sh ./scripts/cartopyPackages.sh >> $PREFIX/make_install_osx.log 2>&1

# statsmodels
sh ./scripts/statsmodelsPackages.sh >> $PREFIX/make_install_osx.log 2>&1

# pygeos
sh ./scripts/pygeosPackages.sh >> $PREFIX/make_install_osx.log 2>&1
