...
# I've been unable to convince Cmake + Ninja to create a dynamic library instead of a bundle. Time for some ugly hacking:
	...
	# 	mkdir -p $PREFIX/build/lib.darwin-arm64-3.13/$directory
	# 	cp ./_skbuild/iphoneos-14.0-arm64-3.13/cmake-build/lib/python3/$file $PREFIX/build/lib.darwin-arm64-3.13/$library
	...
	# 	then 
	# 		install_name_tool -change $PREFIX/Frameworks_iphoneos/lib/libopenblas.dylib @rpath/openblas.framework/openblas  $PREFIX/build/lib.darwin-arm64-3.13/$library 
	# 	fi
	# done
    popd 
    popd 
