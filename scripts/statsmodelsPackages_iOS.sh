...
echo statsmodels libraries for iOS:
find build -name \*.so -print 
echo number of statsmodels libraries for iOS:
...
echo Making a big statsmodels library for iOS:
clang -v -undefined error -dynamiclib \
	  ...
	  build/temp.macosx-*-cpython-313/statsmodels/tsa/statespace/_filters/_inversions.o \
	  -L$PREFIX/Library/lib \
	  -o build/statsmodels.so 
# Make a fwork for this:
install_site_package  statsmodels/statsmodels build/statsmodels.so
pushd build/lib.macosx-*-cpython-313
# Copy this fwork for all the dynamic libraries:
for library in `find statsmodels -name \*.so`
do
	directory=$(dirname $library)
	libname=`basename $library .cpython-313-darwin.so`
	cp $PREFIX/Library/lib/python3.13/site-packages/statsmodels/statsmodels.cpython-313-iphoneos.fwork $PREFIX/Library/lib/python3.13/site-packages/$directory/$libname.cpython-313-iphoneos.fwork
done
# Make a specific framework for the 3 libraries I couldn't merge:
