...
mkdir -p build_ios
rm -rf build_ios/*
env CC=clang CXX=clang++ FC=aarch64-apple-darwin20-gfortran PYODIDE_PACKAGE_ABI=1 meson . build_ios --cross-file ../iphone-osx.meson
pushd build_ios
...
ninja 
echo scikit-learn libraries for iOS:
find sklearn -name \*.so -print
echo number of scikit-learn libraries for iOS: 
find sklearn -name \*.so -print | wc -l 
# 68 libraries by the last count
# create frameworks and .fwork files:
