...
do
	...
	then
		cp -f $file install_mini/$file
	fi
done
mkdir -p install_mini/Library/lib/python3.13/lib-dynload/ >> $PREFIX/make_ios.log 2>&1
cp -f iOS/Frameworks/arm64-iphoneos/lib/python3.13/lib-dynload/*.fwork install_mini/Library/lib/python3.13/lib-dynload/ >> $PREFIX/make_ios.log 2>&1
# And copy the frameworks:
mkdir -p install_mini/iOS/Frameworks  >> $PREFIX/make_ios.log 2>&1
cp -rf ./iOS/Frameworks/arm64-iphoneos/Python.framework install_mini/iOS/Frameworks
for framework in `find install_mini/Library/ -type f -name \*.fwork -exec cat {} \; | cut -f 2 -d "/" | sort -u`
do
	cp -rf iOS/Frameworks/$framework install_mini/iOS/Frameworks/ >> $PREFIX/make_ios.log 2>&1
done
...
do
	...
	then
		cp -f $file install_regular/$file >> $PREFIX/make_ios.log 2>&1
	fi
done
mkdir -p install_regular/Library/lib/python3.13/lib-dynload/ >> $PREFIX/make_ios.log 2>&1
cp -f iOS/Frameworks/arm64-iphoneos/lib/python3.13/lib-dynload/*.fwork install_regular/Library/lib/python3.13/lib-dynload/ >> $PREFIX/make_ios.log 2>&1
# And copy the frameworks:
mkdir -p install_regular/iOS/Frameworks  >> $PREFIX/make_ios.log 2>&1
cp -rf ./iOS/Frameworks/arm64-iphoneos/Python.framework install_regular/iOS/Frameworks
for framework in `find install_regular/Library/ -type f -name \*.fwork -exec cat {} \; | cut -f 2 -d "/" | sort -u`
do
	cp -rf iOS/Frameworks/$framework install_regular/iOS/Frameworks/ >> $PREFIX/make_ios.log 2>&1
done
